...
// Builds a single MCP session credential. The advanced envelope signs the
// session-establishing MCP request and is then reused for later tool calls.
export async function buildMcpSessionAuthHeaders({
  method,
  path,
  body,
  targetAgentDid,
  ...
  logMcpDebug("advanced mcp session canonical", {
    method: method.toUpperCase(),
    path,
    timestamp: timestampMs,
    target_agent_did: targetAgentDid ?? null,
