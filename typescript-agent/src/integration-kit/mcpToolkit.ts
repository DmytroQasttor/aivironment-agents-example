...
export const mcpToolkit = {
  callTool(name: (typeof PLATFORM_MCP_TOOLS)[keyof typeof PLATFORM_MCP_TOOLS], args: Record<string, unknown>, targetAgentDid?: string) {
    return mcpCallTool(name, args, targetAgentDid);
  },
  getTaskLineage(taskId: string, maxParentDepth?: number) {
    return mcpCallTool(PLATFORM_MCP_TOOLS.getTaskLineage, {
      task_id: taskId,
      ...(typeof maxParentDepth === "number" ? { max_parent_depth: maxParentDepth } : {}),
    });
  },
  listRoutes(taskId: string, page?: number, perPage?: number) {
